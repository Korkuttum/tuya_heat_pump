...
def _prettify_code(code):
    """DP code -> human-readable name, e.g. 'green_username' -> 'Green Username'.
    Used as the suggested Name for text-type fields instead of the
    currently-sampled value (which is mutable data, not the field's
    identity)."""
    return str(code).replace("_", " ").replace("-", " ").strip().title()


DICT_NAME_FOR_ENTITY_TYPE = {
    "sensor": "SENSOR_TYPES",
    "switch": "SWITCH_TYPES",
    "number": "NUMBER_TYPES",
    "select": "SELECT_TYPES",
...
def export_snippet(path, labels, dp_code_by_id, encoding_by_dp, device_id):
    # Group entries by entity_type first — each type gets pasted into a
    # different dict in the model file (SENSOR_TYPES, SWITCH_TYPES, ...),
    # so there's no need to repeat an "entity_type" key inside the entry
    # itself: which dict it lives in already says what it is.
    sections = {et: [] for et in DICT_NAME_FOR_ENTITY_TYPE}
    ...
    for key, info in sorted(labels.items(),
        ...
        if entity_type not in sections:
            entity_type = "sensor"
        # "text" fields are the whole raw DP read as a UTF-8 string (e.g.
        # a coffee machine's username), not a packed int at field_index —
        # the encoding written out reflects that instead of the raw
        # int32/int16/uint8 the payload happens to decode as.
        encoding = "utf8_string" if entity_type == "text" else encoding_by_dp.get(int(dp_id), "int32_be")
        scale = info.get("scale", "").strip()
        op = info.get("op", "÷")

        entry = [f'    "{name}": {{']
        entry.append(f'        "dp_id": {dp_id},')
        entry.append(f'        "code": "{name}",')
        entry.append(f'        "raw_source": "{raw_source}",')
        entry.append(f'        "field_index": {idx},')
        entry.append(f'        "encoding": "{encoding}",')
        ...
        if entity_type == "number":
            ...
            if opts:
                ...
                for val, opt_label in opts:
                    entry.append(f'            "{val}": "{opt_label}",')
                entry.append('        },')
        elif entity_type == "text":
            max_len = info.get("max_length", "").strip()
            if max_len:
                entry.append(f'        "max_length": {max_len},')
        # switch needs no extra config — 0/1 is assumed

        entry.append(f'        "name": "{raw_name}",')
        if unit:
            entry.append(f'        "unit": "{unit}",')
        icon = _icon_for_unit(unit)
        if icon:
            entry.append(f'        "icon": "{icon}",')
        if dc:
            entry.append(f'        "device_class": "{dc}",')
    ...
    for et, dict_name in DICT_NAME_FOR_ENTITY_TYPE.items():
        entries = sections[et]
        if not entries:
            continue
        lines.append('')
        lines.append(f'# --- merge into {dict_name} (safe even if already defined above) ---')
        lines.append(f'{dict_name} = globals().get("{dict_name}", {{}})')
        lines.append(f'{dict_name}.update({{')
...
class ExplorerApp:
    def __init__(self, root, creds):
        ...
        self.dp_byte_len = {}  # {dp_id: total raw payload length in bytes} for text max_length
    ...
    def _build_ui(self):
        ...
        # Text — for DPs where the whole raw payload is a UTF-8 string
        # (e.g. a coffee machine's "username" field), not a set of
        # packed int fields. Detected automatically from the string
        # scan; the user just confirms it by picking "text".
        self.text_frame = ttk.Frame(editor)
        self.text_frame.grid(row=4, column=0, columnspan=6, sticky="w", pady=(0, 4))
        ttk.Label(self.text_frame, text="Max length:").pack(side="left")
        self.text_maxlen_var = tk.StringVar(value="")
        ttk.Label(self.text_frame, textvariable=self.text_maxlen_var,
                  foreground="#0a7").pack(side="left", padx=(4, 10))
        ttk.Label(self.text_frame,
                  text="(whole DP payload, UTF-8, null-padded to this length)",
                  foreground="#888").pack(side="left")
    ...
    def _ingest(self, data):
        try:
            ...
            for p in props:
                ...
                bytes_per_field = {"int32_be": 4, "int16_be": 2, "uint8": 1}.get(encoding, 4)
                self.dp_byte_len[p["dp_id"]] = bytes_per_field * len(fields)
    ...
    def _on_select(self, event=None):
        ...
        # Read-only fields, and writable fields with no saved label yet,
        # get a sensible default entity type (mirrors _apply_writable_ui's
        # own default) so we know *before* the name suggestion whether
        # this row is heading for "text".
        saved = self.labels.get(key)
        if saved is not None:
            lbl = saved
            default_et = lbl.get("entity_type", "number")
        else:
            default_et = ("text" if (s_hint and access in ("rw", "wr"))
                          else "number" if access in ("rw", "wr") else "sensor")
            lbl = {
                "name": "", "unit": "", "scale": "", "op": "÷",
                "entity_type": default_et, "min": "", "max": "", "step": "1", "options": "",
            }
        # For "text" fields, the suggested name comes from the DP's own
        # code (e.g. "green_username" -> "Green Username") — never from
        # the currently-sampled string value, since that's mutable data
        # (today it might read "Aykut", tomorrow something else) and
        # must not become part of the entity's identity/export key.
        suggested_name = _prettify_code(code) if default_et == "text" else s_hint
        self.name_var.set(lbl["name"] if lbl["name"] else suggested_name)
        ...
        n_bytes = self.dp_byte_len.get(int(dp))
        self.text_maxlen_var.set(f"{n_bytes} bytes" if n_bytes else "?")
        # "text" is only offered when the string scanner actually found a
        # printable-string run starting at this exact field (s_hint set) —
        # that's how we know the whole DP is a name/username, not just
        # happens-to-be-printable numeric data.
        self._apply_writable_ui(access, has_string=bool(s_hint))
        self.entity_type_var.set(lbl.get("entity_type", self.entity_type_var.get()))
    ...
    def _apply_writable_ui(self, access, has_string=False):
        ...
        if access in ("rw", "wr"):
            ...
            if self.entity_type_var.get() not in opts:
                self.entity_type_var.set("text" if has_string else "number")
        else:
    ...
    def _on_entity_type_change(self):
        """Show the Number min/max/step row, the Select values row, or
        the Text max-length info, matching whichever entity type is
        currently picked. All three stay hidden for read-only fields or
        when Switch is picked (needs no extra config)."""
        ...
        # switch — no extra fields needed
        self._sync_unit_scale_state()

    def _sync_unit_scale_state(self):
        """Unit and Scale don't apply to switch/select/text: a switch is
        always plain 0/1, select's raw value must match its `options`
        dict keys exactly, and text is a name string — none of them are
        a scaled numeric measurement. Hide the whole row for those
        entity types instead of just greying it out, so the panel only
        shows what's relevant."""
        hide = (self._current_access in ("rw", "wr")
                and self.entity_type_var.get() in ("switch", "select", "text"))
        if hide:
    ...
    def _save(self):
        ...
        # Unit/scale are meaningless for switch (0/1), select (raw value
        # must stay untouched to match the options dict keys), and text
        # (a name string, not a scaled measurement).
        if entity_type in ("switch", "select", "text"):
            unit = ""
            scale = ""
        ...
        max_length_v = str(self.dp_byte_len.get(int(vals[0]), "")) if entity_type == "text" else ""
        if name:
            self.labels[key] = {
                "name": name, "unit": unit, "scale": scale, "op": op,
                "entity_type": entity_type, "min": min_v, "max": max_v,
                "step": step_v, "options": options_v, "max_length": max_length_v,
            }
    ...
    def _clear(self):
        ...
        s_hint = self.string_hints.get((int(vals[0]), int(vals[3])), "")
        self._apply_writable_ui(self._current_access, has_string=bool(s_hint))
        self.tree.item(iid, tags=self._tags_for(key, int(vals[5]) if vals[5] not in ("", None) else 0, s_hint))
